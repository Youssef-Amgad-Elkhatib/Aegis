...
While many public notebooks claim 80-90% accuracy on this dataset by exploiting data leakage and biological typos, **Aegis focuses on mathematical integrity, clinical safety, and maximizing Recall (Sensitivity)**. The model achieves a robust **73.21% real-world accuracy**—the theoretical limit (Bayes Error Rate) of this dataset given the vast amount of identical overlapping patient profiles.
...
The dataset contained **18,410 overlapping profiles**—patients with the exact same vitals but different cardiovascular outcomes (The Contradiction Wall).
...
* **Leakage Prevention:** By identifying and handling these clones prior to the `train_test_split`, we prevented identical patient vectors from appearing in both the Training and Testing sets, ensuring the model's test score represents true generalization, not memorization.
...
* **Mean Arterial Pressure (MAP):** `ap_lo + (1/3 * Pulse Pressure)` (Average pressure in a patient's arteries during one cardiac cycle).
* **Health Index:** `(active * 1) - (smoke * 0.5) - (alco * 0.5)` (A composite lifestyle score ranging from -1.0 to 1.0).
* **Cholesterol-Glucose Interaction:** `cholesterol * gluc` (Capturing the compounding exponential risk of metabolic syndrome).
* *Note: Raw continuous variables (`age_in_years`, `BMI`, `weight`, `height`, `ap_hi`) were retained alongside engineered features to allow the Random Forest and Gradient Boosting models to find fine-tuned, multi-dimensional thresholds.*
...
To adapt this model for real-world hospital deployment, CardioSense utilizes `predict_proba` rather than hard binary predictions. By adjusting the diagnostic probability threshold from `0.50` down to `0.35` or `0.30`, clinical directors can instantly boost the model's **Recall to 85%+**, transforming the pipeline from a general classifier into a highly sensitive safety net designed to catch early-stage cardiovascular disease.
